...
<head>
    ...
    <title>Borrar Pozo  <?php echo $rows['nombrePozo'] ?></title>
...
<nav class="IndicePagina">
    ...
    <h1 class="titulo-Seccion">Borrar Pozo</h1> <br> <br>
    <form action="" method="POST">
    <label class="form-label"> ¿Está seguro que desea borrar el pozo con los siguientes datos? </label><br><br>
    <label class="form-label"> Nombre del Pozo: <b><?php echo $rows['nombrePozo'] ?></b> </label> <br>
    <label class="form-label"> Ciudad del Pozo: <b><?php echo $rows['ciudad'] ?></b> </label> <br> <br>
...
if(isset($_POST['botonBorrar'])) {
    $queryBorrarRegistros = "DELETE from registro where Pozo_IdPozo=$id";
    $resultadoRegistro = mysqli_query($conn,$queryBorrarRegistros);
    $queryBorrar = "DELETE from pozo where idPozo=$id";
